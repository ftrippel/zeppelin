...
public class BigQueryInterpreter extends Interpreter {
    ...
    static final String LOCATION = "zeppelin.bigquery.location";
    ...
    private InterpreterResult executeSql(String sql) {
        ...
        String location = getProperty(LOCATION, "US");
        ...
        switch (sqlDialect) {
            ...
            default:
                ...
                try (final BufferedReader bufferedReader = new BufferedReader(new StringReader(sql))) {
                    String line = null;
                    while ((line = bufferedReader.readLine()) != null) {
                        if (line.startsWith("#standardSQL")) {
                            useLegacySql = false;
                        } else if (line.startsWith("#legacySQL")) {
                            useLegacySql = true;
        ...
        try {
            pages = run(sql, projId, location, wTime, maxRows, useLegacySql);
        } catch (IOException ex) {
            logger.error(ex.getMessage());
            return new InterpreterResult(Code.ERROR, ex.getMessage());
        }
